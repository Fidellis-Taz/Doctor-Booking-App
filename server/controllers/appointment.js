...
exports.attended = async (req, res) => {
    ...
    //when the doc has attended to the patient 
    /*    try {
           const appointmentId = req.params.appointmentId;
           const attendedAppointment = await Appointment.findOneAndUpdate(appointmentId, { appointmentStatus: "Past" }, { new: true })
   
           res.json({ successMessage: ` Appointment was successfully edited`, attendedAppointment });
   
       } catch (err) {
           console.log(err, 'attendedAppointment.readAll error');
           res.status(500).json({
               errorMessage: 'Please try again later',
           });
       } */
